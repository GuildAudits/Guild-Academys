...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  children,
  color = '#5227FF',
  // Lower defaults to reduce GPU/CPU load while preserving look
  speed = 0.6,
  chaos = 0.6,
  thickness = 1,
  className,
  ...
  const reduceAnim = useMemo(() => {
    if (typeof window === 'undefined') return true
    const prefersReduced = typeof window.matchMedia === 'function' &&
      window.matchMedia('(prefers-reduced-motion: reduce)').matches
    // Only respect explicit user preference; otherwise keep motion on.
  ...
  const updateAnim = () => {
    ...
    // Disable heavy SVG filter when reduced motion is preferred or on constrained devices
    if (strokeRef.current) {
      strokeRef.current.style.filter = reduceAnim ? 'none' : `url(#${filterId})`
    }

    if (reduceAnim) return
    ...
    // Slow down animation to reduce CPU
    const baseDur = 10
    const dur = Math.max(0.001, baseDur / (speed || 1))
    ;[...dyAnims, ...dxAnims].forEach(a => a.setAttribute('dur', `${dur}s`))

    const disp = svg.querySelector('feDisplacementMap')
    // Reduce displacement intensity significantly
    if (disp) disp.setAttribute('scale', String(12 * (chaos || 1)))

    const filterEl = svg.querySelector<SVGFilterElement>(`#${CSS.escape(filterId)}`)
    if (filterEl) {
      // Smaller filter region to reduce work while keeping visuals correct
  ...
  const glow1Style: CSSProperties = {
    ...
    filter: `blur(${0.4 + thickness * 0.2}px)`,
    opacity: 0.35
  }
  ...
  const glow2Style: CSSProperties = {
    ...
    borderStyle: 'solid',
    borderColor: color,
    filter: `blur(${1.2 + thickness * 0.4}px)`,
    opacity: 0.35
  }
      ...
      >
        <defs>
          <filter id={filterId} colorInterpolationFilters="sRGB" x="-20%" y="-20%" width="140%" height="140%">
            {/* Lower turbulence complexity for performance */}
            <feTurbulence type="turbulence" baseFrequency="0.01" numOctaves="3" result="noise1" seed="1" />
            <feOffset in="noise1" dx="0" dy="0" result="offsetNoise1">
              <animate attributeName="dy" values="700; 0" dur="6s" repeatCount="indefinite" calcMode="linear" />
            </feOffset>

            <feTurbulence type="turbulence" baseFrequency="0.01" numOctaves="3" result="noise2" seed="1" />
            <feOffset in="noise2" dx="0" dy="0" result="offsetNoise2">
              <animate attributeName="dy" values="0; -700" dur="6s" repeatCount="indefinite" calcMode="linear" />
            </feOffset>

            <feTurbulence type="turbulence" baseFrequency="0.01" numOctaves="3" result="noise1" seed="2" />
            <feOffset in="noise1" dx="0" dy="0" result="offsetNoise3">
              <animate attributeName="dx" values="490; 0" dur="6s" repeatCount="indefinite" calcMode="linear" />
            </feOffset>

            <feTurbulence type="turbulence" baseFrequency="0.01" numOctaves="3" result="noise2" seed="2" />
